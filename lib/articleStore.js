...
export async function getMergedArticles({ window = '24h', category = 'all', limit = 50 } = {}) {
  const [hnData, redditData, blueskyData] = await redis.mget(
    'articles:hn', 'articles:reddit', 'articles:bluesky'
  );
  ...
  for (const [url, article] of Object.entries(hnArticles)) {
    merged[url] = {
      ...
      sources: {
        hackerNews: article.sourceData || { count: 0, score: 0, itemIds: [] },
  ...
  // Merge Reddit articles
  for (const [url, article] of Object.entries(redditArticles)) {
    if (merged[url]) {
      merged[url].sources.reddit = article.sourceData || { count: 0, score: 0, subreddits: [] };
      ...
      merged[url] = {
        ...
        sources: {
          ...
          reddit: article.sourceData || { count: 0, score: 0, subreddits: [] },
          bluesky: { count: 0, postUris: [] },
        },
      };
    }
  ...
  // Merge Bluesky articles
  for (const [url, article] of Object.entries(blueskyArticles)) {
    if (merged[url]) {
      merged[url].sources.bluesky = article.sourceData || { count: 0, postUris: [] };
      if ((article.title || '').length > (merged[url].title || '').length) {
        merged[url].title = article.title;
        merged[url].category = article.category;
      }
      ...
      merged[url] = {
        ...
        sources: {
          hackerNews: { count: 0, score: 0, itemIds: [] },
          reddit: { count: 0, score: 0, subreddits: [] },
          bluesky: article.sourceData || { count: 0, postUris: [] },
        },
      };
    }
  }
  ...
  articles.sort((a, b) => {
    ...
    const scoreA = (a.sources.hackerNews.score || 0) + (a.sources.reddit.score || 0);
    const scoreB = (b.sources.hackerNews.score || 0) + (b.sources.reddit.score || 0);
